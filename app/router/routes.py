...
def setup_routers(app: FastAPI) -> None:
    ...
    app.include_router(ai_log_routes.router)
...
def setup_page_routes(app: FastAPI) -> None:
    ...
    async def ai_logs_page(request: Request):
        ...
        try:
            auth_token = request.cookies.get("auth_token")
            if not auth_token or not verify_auth_token(auth_token):
                logger.warning("Unauthorized access attempt to AI logs page")
                return RedirectResponse(url="/", status_code=302)

            logger.info("AI logs page accessed successfully")
            return templates.TemplateResponse("ai_logs.html", {"request": request})
        except Exception as e:
            logger.error(f"Error accessing AI logs page: {str(e)}")
            raise
