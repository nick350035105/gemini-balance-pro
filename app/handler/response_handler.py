...
def _handle_gemini_stream_response(
    ...
    # 安全检查：确保 candidates 存在且不为空
    if not response.get("candidates"):
        logger.warning(f"No candidates in stream response for model: {model}, creating default structure")
        response["candidates"] = [{"content": content, "index": 0}]
    else:
...
def _handle_gemini_normal_response(
    ...
    # 安全检查：确保 candidates 存在且不为空
    if not response.get("candidates"):
        logger.warning(f"No candidates in normal response for model: {model}, creating default structure")
        response["candidates"] = [{"content": content, "index": 0}]
    else:
        response["candidates"][0]["content"] = content
    return response
