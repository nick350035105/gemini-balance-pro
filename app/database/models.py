...
class AILog(Base):
    """
    AI 请求/响应日志表，记录完整的 API 调用信息
    """
    __tablename__ = "t_ai_logs"

    id = Column(Integer, primary_key=True, autoincrement=True)
    request_time = Column(DateTime, default=datetime.datetime.now, index=True, comment="请求时间")
    ...
    # 额外信息
    model_name = Column(String(100), nullable=True, comment="模型名称")
    api_key = Column(String(100), nullable=True, comment="API密钥(脱敏)")
    ...
    # 内容预览
    content_preview = Column(String(100), nullable=True, comment="请求内容预览")

    def __repr__(self):
        return f"<AILog(id='{self.id}', path='{self.path}', status='{self.status_code}')>"
