...
def extract_exception_info(e: Exception, default_status_code: int = 500) -> Tuple[int, str]:
    """
    安全地从异常中提取状态码和错误信息。

    兼容两种异常格式:
    1. 自定义格式: Exception(status_code, error_message)
    2. 标准格式: Exception(message) 或其他单参数/无参数异常

    Args:
        e: 捕获的异常
        default_status_code: 当无法提取状态码时使用的默认值

    Returns:
        Tuple[int, str]: (状态码, 错误信息)
    """
    if len(e.args) >= 2:
        # 自定义异常格式: Exception(status_code, error_message)
        try:
